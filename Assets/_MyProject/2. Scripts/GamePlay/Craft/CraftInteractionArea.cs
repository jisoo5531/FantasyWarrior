...
public class CraftInteractionArea : MonoBehaviour
{
    ...
    {
        Debug.Log($"어떤? " + userid);
        UserCraftToolData userCraftTool = GameManager.Instance.invenManager[userid].userCraftToolClient.Find(x => x.CreftType.Equals(craftType));        
    ...
    {                
        ...
        {
            Debug.Log("아이디 :ㄴㅇㅁㄹ " + other.gameObject.GetComponent<PlayerController>().userID);
            int userId = DatabaseManager.Instance.GetPlayerData(other.gameObject).UserId;
            CheckUserHaveTool(userId);
